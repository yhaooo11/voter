...
def generate_candidate_data(candidate_number, total_parties):
    res = requests.get(f'{RANDOM_USER_BASE_URL}&gender={("female" if candidate_number % 2 == 1 else "male")}')
    ...
    if res.status_code == 200:
        ...
        return {
            ...
            'biography': 'fake bio',
            'campaign_platform': 'campaign info, party promises',
            'photo_url': user_data['picture']['large']
        }
    else:
        return "Error fetching mock data"
    
def insert_candidate(conn, cur, voter_data):
    cur.execute("""
            INSERT INTO candidates (candidate_id, candidate_name, party, biography, campaign_platform, photo_url)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (
        candidate['candidate_id'], candidate['candidate_name'], candidate['party'], candidate['biography'],
        candidate['campaign_platform'], candidate['photo_url']))
    
    conn.commit()

def generate_voter_data():
    res = requests.get(RANDOM_USER_BASE_URL)
